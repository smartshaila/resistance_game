module PlayerAssignmentsHelper
  def scoreboard_table game
    capacities = ([['Mission #', 'Capacity', 'Rejected Teams', 'Result']] | game.mission_capacities.map{|c|
      ...
      [c.mission_number + 1, c.capacity, (mission.rejection_count unless mission.nil?), ("<span class='#{(mission.result.nil? ? 'glyphicon glyphicon-time' : (mission.result ? 'glyphicon glyphicon-ok' : 'glyphicon glyphicon-remove'))}'></span>" unless mission.nil?)]
    }).transpose
